...
test('Result concat properties (Semigroup)', t => {
  ...
  t.ok(concat.associativity(equals, ra, rb, rc), 'Ok associativity')
  t.ok(concat.associativity(equals, la, lb, lc), 'Err associativity')
...
test('Result map properties (Functor)', t => {
  ...
  t.ok(map.identity(equals, Ok(undefined)), 'Ok identity')
  t.ok(map.composition(equals, f, g, Ok(44)), 'Ok composition')

  t.ok(map.identity(equals, Err(NaN)), 'Err identity')
  t.ok(map.composition(equals, f, g, Ok(77)), 'Err composition')
...
test('Result ap properties (Apply)', t => {
  ...
  const extractApply = (m, v) =>
    m.either(identity, identity)(v)

  const leftEq = (a, b) =>
    extractApply(a, 10) === extractApply(b, 10)
  ...
  t.ok(ap.composition(equals, rg, rf, rv), 'Ok composition')
  t.ok(ap.composition(leftEq, lg, lf, lv), 'Err composition')
...
test('Result chain properties (Chain)', t => {
  ...
  t.ok(chain.associativity(equals, rf, rg, rv), 'Ok associativity')
  t.ok(chain.associativity(equals, lf, lg, lv), 'Err associativity')
